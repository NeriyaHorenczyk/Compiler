...
pub fn main() !void {
    ...
    while (try it.next()) |entry| {
        ...
        if (!mem.eql(u8, content, "")) {
            ...
            try code_generator._class(allocator, writer, tokens, &current, &class_table, &static_counter, &field_counter, &label_counter, clear_file_name[0 .. clear_file_name.len - 1]);
        }
    }
}
