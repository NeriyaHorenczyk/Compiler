...
fn insert_into_class_symbol_table(allocator: std.mem.Allocator, class_table: *ClassMap, var_type: []const u8, is_static: bool, index: *usize, var_name: []const u8) anyerror!void {
    ...
    class_table.put(var_name, record_ptr.*) catch unreachable;
...
// helping function to extract the type from some record in the symbol tables
fn extract_type_from_symbol_tables(class_table: *ClassMap, function_table: *FunctionMap, var_name: []const u8) anyerror!?[]const u8 {
    if (function_table.get(var_name)) |function_record| {
        return function_record.var_type;
    } else if (class_table.get(var_name)) |class_record| {
        return class_record.var_type;
    }
    return null;
}

// helping function to debug and print the class symbol table
fn debug_class_symbol_table(class_table: *ClassMap) void {
    std.debug.print("Class Symbol Table:\n", .{});
    var it = class_table.iterator();
    while (it.next()) |entry| {
        std.debug.print("Variable: {s}, Type: {s}, is_static: {}, Index: {d}\n", .{
            entry.key_ptr.*,
            entry.value_ptr.var_type,
            entry.value_ptr.static,
            entry.value_ptr.index,
        });
    }
}

// helping function to debug and print the function symbol table
fn debug_function_symbol_table(function_table: *FunctionMap) void {
    std.debug.print("Function Symbol Table:\n", .{});
    var it = function_table.iterator();
    while (it.next()) |entry| {
        std.debug.print("Variable: {s}, Type: {s}, is_argument: {}, Index: {d}\n", .{
            entry.key_ptr.*,
            entry.value_ptr.var_type,
            entry.value_ptr.argument,
            entry.value_ptr.index,
        });
    }
}

// helping function to get the number of fields in the class symbol table
fn get_num_fields(class_table: *ClassMap) usize {
    var num_fields: usize = 0;
    var it = class_table.iterator();
    while (it.next()) |entry| {
        if (!entry.value_ptr.static) {
            num_fields += 1;
        }
    }
    return num_fields;
...
fn _ifStatement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, label_counter: *usize, class_name: []const u8) anyerror!void {
    ...
    try _expression(allocator, writer, tokens_list, current, class_table, function_table, class_name);
    ...
    try _statements(allocator, writer, tokens_list, current, class_table, function_table, label_counter, class_name);
...
fn _letStatement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
...
fn _whileStatement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, label_counter: *usize, class_name: []const u8) anyerror!void {
    const current_label = label_counter.*;
...
fn _statement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, label_counter: *usize, class_name: []const u8) anyerror!void {
    if ((try peek(tokens_list, (current.*))).equals(tokens.if_kw)) {
        try _ifStatement(allocator, writer, tokens_list, current, class_table, function_table, label_counter, class_name);
    } else {
        if ((try peek(tokens_list, (current.*))).equals(tokens.let_kw)) {
            try _letStatement(allocator, writer, tokens_list, current, class_table, function_table, class_name);
        } else {
            if ((try peek(tokens_list, (current.*))).equals(tokens.while_kw)) {
                try _whileStatement(allocator, writer, tokens_list, current, class_table, function_table, label_counter, class_name);
            } else {
                if ((try peek(tokens_list, (current.*))).equals(tokens.do_kw)) {
                    try _doStatement(allocator, writer, tokens_list, current, class_table, function_table, class_name);
                } else {
                    if ((try peek(tokens_list, (current.*))).equals(tokens.return_kw)) {
                        try _returnStatement(allocator, writer, tokens_list, current, class_table, function_table, class_name);
                    }
                }
            }
        }
...
fn _statements(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, label_counter: *usize, class_name: []const u8) anyerror!void {
    while ((try peek(tokens_list, (current.*))).equals(tokens.if_kw) or (try peek(tokens_list, (current.*))).equals(tokens.let_kw) or (try peek(tokens_list, (current.*))).equals(tokens.while_kw) or (try peek(tokens_list, (current.*))).equals(tokens.do_kw) or (try peek(tokens_list, (current.*))).equals(tokens.return_kw)) {
        try _statement(allocator, writer, tokens_list, current, class_table, function_table, label_counter, class_name);
    }
}

fn _subroutineCall(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
    if ((try peek(tokens_list, (current.*) + 1)).equals(tokens.lparen) and (try peek(tokens_list, (current.*))).equals(tokens.identifier)) {
        ...
        var num_arguments = try _expressionList(allocator, writer, tokens_list, current, class_table, function_table, class_name);

        num_arguments += 1; // we need to add the 'this' pointer

        code = try std.fmt.allocPrint(allocator, "call {s}.{s} {d}\n", .{ class_name, sub_routine_name, num_arguments });
        try writeCode(writer, code);
        allocator.free(code);
        ...
        if ((try peek(tokens_list, (current.*) + 1)).equals(tokens.dot) and (try peek(tokens_list, (current.*))).equals(tokens.identifier)) {
            ...
            if (var_index != null and var_segment != null) {
                ...
                num_arguments += try _expressionList(allocator, writer, tokens_list, current, class_table, function_table, class_name);

                code = try std.fmt.allocPrint(allocator, "call {s}.{s} {d}\n", .{ (try extract_type_from_symbol_tables(class_table, function_table, var_name)).?, sub_routine_name, num_arguments });
                try writeCode(writer, code);
                allocator.free(code);
...
fn _term(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
            ...
            if ((try peek(tokens_list, (current.*))).equals(tokens.true_kw) or (try peek(tokens_list, (current.*))).equals(tokens.false_kw) or (try peek(tokens_list, (current.*))).equals(tokens.null_kw) or (try peek(tokens_list, (current.*))).equals(tokens.this_kw)) {
                ...
                if ((try peek(tokens_list, (current.*))).equals(tokens.lparen)) {
                    ...
                    if (((try peek(tokens_list, (current.*) + 1)).equals(tokens.lparen) and (try peek(tokens_list, (current.*))).equals(tokens.identifier)) or ((try peek(tokens_list, (current.*) + 1)).equals(tokens.dot) and (try peek(tokens_list, (current.*))).equals(tokens.identifier))) {
                        try _subroutineCall(allocator, writer, tokens_list, current, class_table, function_table, class_name);
                    } else {
                        if ((try peek(tokens_list, (current.*) + 1)).equals(tokens.lbracket) and (try peek(tokens_list, (current.*))).equals(tokens.identifier)) {
                            ...
                            if ((try peek(tokens_list, (current.*))).equals(tokens.tilde) or (try peek(tokens_list, (current.*))).equals(tokens.minus)) {
                                ...
                                try _term(allocator, writer, tokens_list, current, class_table, function_table, class_name);
...
fn _expression(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
    try _term(allocator, writer, tokens_list, current, class_table, function_table, class_name);
...
fn _doStatement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
...
fn _returnStatement(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!void {
...
fn _expressionList(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, class_name: []const u8) anyerror!usize {
...
fn _classVarDec(allocator: std.mem.Allocator, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, static_counter: *usize, field_counter: *usize) anyerror!void {
    ...
    try insert_into_class_symbol_table(allocator, class_table, var_type, is_static, if (is_static) static_counter else field_counter, var_name);
...
pub fn _subroutineDec(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, label_counter: *usize, class_name: []const u8) anyerror!void {
    ...
    if (current_token.equals(tokens.constructor_kw)) {
        is_constructor = true;
    }
    ...
    try _parameterList(tokens_list, current, &function_table, &num_arguments);

    // ignore the ')'
    try proceed(current);

    try _subroutineBody(allocator, writer, tokens_list, current, class_table, &function_table, label_counter, function_name, is_method, is_constructor, class_name);
}

fn _parameterList(tokens_list: std.ArrayList(Token), current: *usize, function_table: *FunctionMap, num_arguments: *usize) anyerror!void {
    const current_token = (try peek(tokens_list, (current.*)));
...
fn _subroutineBody(allocator: std.mem.Allocator, writer: anytype, tokens_list: std.ArrayList(Token), current: *usize, class_table: *ClassMap, function_table: *FunctionMap, label_counter: *usize, function_name: []const u8, is_method: bool, is_constructor: bool, class_name: []const u8) anyerror!void {
